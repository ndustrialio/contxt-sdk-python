...
class TestFacilitiesService:
    ...
    def test_get_facility_with_id(self, facility_id: int = TestFacility.id):
        facility = self.service.get_facility_with_id(facility_id)
        assert facility
        assert facility.id == facility_id

    def test_get_facility_with_name(self, facility_name: str = TestFacility.name):
        facility = self.service.get_facility_with_name(facility_name)
        assert facility
        assert facility.name.lower() == facility_name.lower()

    def test_get_facility_with_asset_id(self, asset_id: str = TestFacility.asset_id):
