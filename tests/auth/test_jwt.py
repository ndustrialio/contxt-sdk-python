import jwt
import pytest

from contxt.auth.jwt import ContxtTokenValidator, InvalidTokenError, TokenValidator

pytest.importorskip("cryptography", reason="Requires cryptography")


@pytest.fixture
def public_key():
    return """\
-----BEGIN PUBLIC KEY-----
MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQC1MZ30SIuWmPgbZcjhoyjauGw4
mkzuds/QPGxqhykAs5+3iIzgzJSLehb9LLrGNzufl2OPu5Bza8LEXpprO8sevajf
yOQ45SZ1Kbm0ILrYaLfl1yvFr6MqHrO07HWsiW9zoHDFU31sd2CoGJsu35fTils1
/9Cq7+oWErNfJ61ZPQIDAQAB
-----END PUBLIC KEY-----
"""


@pytest.fixture
def private_key():
    return """\
-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
"""


def test_token_validator(public_key, private_key):
    claims = {"foo": "bar", "aud": "foo_audience", "iss": "foo_issuer"}
    validator = TokenValidator(
        audience=claims["aud"], issuer=claims["iss"], public_key=public_key, algorithms=["RS256"]
    )
    token = jwt.encode(claims, private_key, algorithm="RS256")
    assert validator.validate(token) == claims


@pytest.mark.skip(reason="Mock this instead")
def test_contxt_token_validator():
    token_validator = ContxtTokenValidator(audience="foo_audience")
    with pytest.raises(InvalidTokenError):
        token_validator.validate("bad_token")
