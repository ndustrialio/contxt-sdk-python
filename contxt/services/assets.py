...
class AssetsService(ConfiguredApi):
    ...
    def get_metric_values(
        self, asset_id: str, metric_id: Optional[str] = None, params: Optional[Dict] = None, page_options: Optional[PageOptions] = None
    ) -> Iterable[MetricValue]:
        assert params is None or \
            sum([key not in ["effective_end_date", "effective_start_date"] for key in params]) == 0, \
        ...
        return PagedRecords(api=self, url=url, params=params, options=page_options, record_parser=MetricValue.from_api)
