...
def get(clients: Clients, org: str, fields: List[str], sort: str, cluster_slug: Optional[str]) -> None:
    ...
    items = [clients.contxt_deployments.get_cluster(organization.id, cluster_slug)] if cluster_slug else clients.contxt_deployments.get_clusters(organization.id)
...
def login(clients: Clients, host: str, org: str) -> None:
    ...
    try:
        ...
        kubeconfig = {
            ...
            "contexts": [
                {"name": cluster, "context": {"cluster": cluster, "namespace": "default", "user": cluster}}
            ],
        }
